        ...
        <View style={styles.modalContainer}>
          <ScrollView>
            ...
            <View style={[styles.statusBadge, deliveryItem.status === "cancelled" ? styles.statusCancelled : styles.statusActive]}>
              <Text style={styles.statusText}>{deliveryItem.status === "cancelled" ? "종료됨" : "진행 중"}</Text>
            </View>
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.value}>{deliveryItem.items[0]?.description || "없음"}</Text>
            </View>

            {/* 배달 정보 */}
            <View style={styles.section}>
              <Text style={styles.label}>배달 위치</Text>
              <Text style={styles.value}>{deliveryItem.resolvedAddress}</Text>

              <Text style={styles.label}>상세 배달 위치</Text>
              <Text style={styles.value}>AI융합대학 3층</Text>

              <Text style={styles.label}>배달 유형</Text>
              <Text style={styles.value}>{deliveryItem.deliveryType === "direct" ? "직접 배달" : "기타"}</Text>

              <Text style={styles.label}>배달비</Text>
              <Text style={styles.value}>{deliveryItem.deliveryFee}원</Text>

              <Text style={styles.label}>총 가격</Text>
              <Text style={styles.value}>{deliveryItem.price || 0}원</Text>
