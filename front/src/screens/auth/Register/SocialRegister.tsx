...
import { register, socialRegisterAction } from '../../../redux/actions/userAction';
    ...
    <ScrollView style={{ padding: 20 }}>
        ...
        <View style={styles.bulletList}>
            ...
            <View style={[styles.bulletList, styles.nestedList]}>
                <Text style={styles.bulletItem}>• 러너 매칭 전 취소: 과금 안 됨</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 5분 이내 취소: 전액 환불</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 5분 초과 ~ 10분 이내 취소: 1000원 과금 후 서비스비 전액 환불</Text>
                <Text style={styles.bulletItem}>• 러너매칭 후 10분 초과 ~ 20분 이내 취소: 3,000원 과금 후 서비스비 전액 환불</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 20분 초과 취소: 5,000원 과금 후 서비스 비 전액 환불</Text>
                <Text style={styles.bulletItem}>• 단, 주문 금액이 패널티 금액을 초과할 시 전액 과금 됨.</Text>
            </View>
            <Text style={styles.bulletItem}>C. 시간 예약 서비스의 경우</Text>
            <View style={[styles.bulletList, styles.nestedList]}>
                <Text style={styles.bulletItem}>• 러너 매칭 전 취소: 과금 없음</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 서비스 시간 기준 2시간 이전 취소: 과금 없음</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 서비스 시간 기준 1시간 ~ 2시간 전 취소: 1,000원 과금 후 서비스비 전액 환불</Text>
                <Text style={styles.bulletItem}>• 러너 매칭 후 서비스 시간 기준 1시간 이내 취소: 3,000원 과금 후 서비스비 전액 환불</Text>
                <Text style={styles.bulletItem}>• 서비스 시간 이후 취소 또는 무단 미이용: 전액 과금</Text>
                <Text style={styles.bulletItem}>• 단, 주문 금액이 패널티 금액을 초과할 시 전액 과금 됨.</Text>
            </View>
        </View>
        <Text style={styles.termsText}>
            A. 서비스 수행 중 러너가 서비스 취소 시 - 고객: 전액 환불 - 러너: 1회 경고 후 2회 경고부터 3일 서비스 이용 제한
        </Text>
...
const SocialRegisterScreen = ({ route }: FourthScreenProps) => {
    ...
    // 입력값이 모두 채워졌는지
    const inputFilled = !!username.trim() && !!nickname.trim();
    ...
    const handleNext = async () => {
        if (!allRequiredChecked || !inputFilled || isLoading) return;
        setIsLoading(true);
        try {
            await dispatch(socialRegisterAction(nickname, username, email));
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.card} contentContainerStyle={{ paddingBottom: 20 }} showsVerticalScrollIndicator={false}>
                <TouchableOpacity style={styles.allRow} onPress={toggleAll}>
                    <Ionicons name={allChecked ? 'checkbox' : 'square-outline'} size={22} color={allChecked ? '#3384FF' : '#666'} />
                    <Text style={styles.allText}>약관 전체동의</Text>
                </TouchableOpacity>
                <View style={styles.divider} />
                {TERMS.map(t => (
                    <View key={t.key} style={styles.termRow}>
                        <TouchableOpacity onPress={() => toggleTerm(t.key)} style={styles.checkHit}>
                            <Ionicons name={terms[t.key] ? 'checkbox' : 'square-outline'} size={20} color={terms[t.key] ? '#3384FF' : '#888'} />
                        </TouchableOpacity>
            ...
            <TouchableOpacity
                style={[
                    styles.nextButton,
                    (!allRequiredChecked || !inputFilled || isLoading) && styles.nextDisabled
                ]}
                disabled={!allRequiredChecked || !inputFilled || isLoading}
                onPress={handleNext}
            >
                {isLoading ? (
